function validar_curso(form) {
    ...
    let certificado_plantilla = form.certificado_plantilla;
    ...
    if (isFull(certificado_plantilla)) {
        setDesError(certificado_plantilla);
    } else {
        setError(certificado_plantilla);
    }
    if (
        isFull(nombre) &&
        isFull(fecha_inicio) &&
        isFull(fecha_fin) &&
        isFull(numero_cupos) &&
        isFull(link_whatsapp) &&
        isFull(id_modelo_curso) &&
        isFull(certificado_plantilla)
    ) {
        return true;
    } else {
        return false;
